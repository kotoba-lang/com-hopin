(ns hopin.main
  "Kotodama WASM entrypoint for the Hopin clean-room actor (L4) — Clojure port.
  ...
  facts (`hopin.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"hopin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hopin-compat" :tier tier :entities entities} 200])
